...
output = open("../src/shared/glapi.h", "w")
...
for name, group in group_dict.iteritems():
    ...
    for vn, vv in group.vals.iteritems():
        e = GroupEntry(vn, vv)
        e.versions = versions[versions.index(group.minvers[vn]):]
        g.entries.append(e)
    gl_groups[name] = g

for name, func in func_dict.iteritems():
    params = [FuncParam(p.name, p.group) for p in func.params]
    f = Function(params)
    if func.minver != None:
        f.versions = versions[versions.index(func.minver):]
    gl_functions[name] = f
...
i = 0
for name, group in gl_groups.iteritems():
    for entry in group.entries:
        ...
        output.write("static const glapi_group_entry_t entry_%d = {&req_%d, %du, \"%s\"};\n" %
                     (id(entry),
                      next_req_id,
                      entry.value,
                      entry.name))
    ...
    output.write("static const glapi_group_t group_%d = {%s, \"%s\", %d, entries_%d};\n" %
                 (id(group),
                  "true" if group.bitmask else "false",
                  name,
                  len(group.entries),
                  i))
...
for name, func in gl_functions.iteritems():
    for arg in func.params:
        group = ("&group_%d" % id(gl_groups[arg.group])) if arg.group != None else "NULL"
...
output.write("static glapi_group_t*const* groups[] = {%s};\n" %
             ", ".join(["&group_%d" % id(group) for group in gl_groups.values()]))

output.write("static glapi_function_t** funcs[] = {%s};\n" %
             ", ".join(["&func_%d" % id(func) for func in gl_functions.values()]))
