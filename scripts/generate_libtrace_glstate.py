...
for prop in properties:
    ...
    #if prop.map:
    #    prop.get_keys_func_sig = 'trc_gl_state_key_t trc_gl_state_get_%s_key(trace_t* trace, size_t index);' % prop.name
...
#print 'typedef struct trc_gl_state_key_t {'
#print '    const char* name;'
#print '    uint key;'
#print '} trc_gl_state_key_t;'
...
"""for prop in properties:
    if not prop.map: continue
    
    print prop.get_keys_func_sig + ' {'
    print '    switch (index) {'
    for i, key in zip(range(len(prop.map_keys)), prop.map_keys):
        print '    case %d: return (trc_gl_state_key_t){"%s", %s}' % (i, key, key)
    print '    default: return (trc_gl_state_key_t){NULL, 0};'
    print '    }'
    print '}'
    print"""
