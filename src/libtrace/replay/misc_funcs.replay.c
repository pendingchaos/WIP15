...
static void pixel_store(GLenum pname, GLint param) {
    ...
    case GL_UNPACK_LSB_FIRST: gls_set_state_bool(pname, 0, param!=0); break;
    ...
    case GL_UNPACK_SKIP_IMAGES:
        ...
        gls_set_state_int(pname, 0, param);
        break;
...
glSampleMaski: //GLuint p_maskNumber, GLbitfield p_mask
    if (p_maskNumber >= gls_get_state_int(GL_MAX_SAMPLE_MASK_WORDS, 0))
        ERROR("Invalid mask number");
    real(p_maskNumber, p_mask);
    union {int32_t maski; uint32_t masku;} u;
    u.masku = p_mask;
    gls_set_state_int(GL_SAMPLE_MASK_VALUE, p_maskNumber, u.maski);
...
glDrawBuffers: //GLsizei p_n, const GLenum* p_bufs
    ...
    if (p_n > gls_get_state_int(GL_MAX_DRAW_BUFFERS, 0))
        ERROR("The buffer count is greater than GL_MAX_DRAW_BUFFERS");
    
    trc_obj_t* fb = gls_get_draw_framebuffer();
    
    uint color_min = GL_COLOR_ATTACHMENT0;
    uint color_max = GL_COLOR_ATTACHMENT0 + gls_get_state_int(GL_MAX_COLOR_ATTACHMENTS, 0);
    ...
    if (!fb) {
        gls_state_enum_init(GL_DRAW_BUFFER, p_n, p_bufs);
    } else {
...
glEnable: //GLenum p_cap
    ...
    if ((p_cap&0x3000)==0x3000 && p_cap-0x3000<gls_get_state_int(GL_MAX_CLIP_DISTANCES, 0))
        gls_set_enabled(GL_CLIP_DISTANCE0, p_cap-0x3000, true);
    else
...
glEnablei: //GLenum p_target, GLuint p_index
    if ((p_target&0x3000) == 0x3000) {
        if (p_index==0 && p_target-0x3000<gls_get_state_int(GL_MAX_CLIP_DISTANCES, 0))
            gls_set_enabled(GL_CLIP_DISTANCE0, p_target-0x3000, true);
    } else if (p_index < gls_get_enabled_size(p_target)) {
        gls_set_enabled(p_target, p_index, true);
    }
...
glDisablei: //GLenum p_target, GLuint p_index
    if ((p_target&0x3000) == 0x3000) {
        if (p_index==0 && p_target-0x3000<gls_get_state_int(GL_MAX_CLIP_DISTANCES, 0))
            gls_set_enabled(GL_CLIP_DISTANCE0, p_target-0x3000, false);
    } else if (p_index < gls_get_enabled_size(p_target)) {
        gls_set_enabled(p_target, p_index, false);
    }
    real(p_target, p_index);

glDepthMask: //GLboolean p_flag
    gls_set_state_bool(GL_DEPTH_WRITEMASK, 0, p_flag);
    real(p_flag);

glColorMask: //GLboolean p_red, GLboolean p_green, GLboolean p_blue, GLboolean p_alpha
    for (size_t i = 0; i < gls_get_state_int(GL_MAX_DRAW_BUFFERS, 0); i+=4) {
        gls_set_state_bool(GL_COLOR_WRITEMASK, i+0, p_red);
        gls_set_state_bool(GL_COLOR_WRITEMASK, i+1, p_green);
        gls_set_state_bool(GL_COLOR_WRITEMASK, i+2, p_blue);
        gls_set_state_bool(GL_COLOR_WRITEMASK, i+3, p_alpha);
    }
    real(p_red, p_green, p_blue, p_alpha);

glColorMaski: //GLuint p_index, GLboolean p_r, GLboolean p_g, GLboolean p_b, GLboolean p_a
    if (p_index < gls_get_state_int(GL_MAX_DRAW_BUFFERS, 0)) {
        gls_set_state_bool(GL_COLOR_WRITEMASK, p_index*4+0, p_r);
        gls_set_state_bool(GL_COLOR_WRITEMASK, p_index*4+1, p_g);
        gls_set_state_bool(GL_COLOR_WRITEMASK, p_index*4+2, p_b);
        gls_set_state_bool(GL_COLOR_WRITEMASK, p_index*4+3, p_a);
    }
...
glStencilMask: //GLuint mask
    union {int32_t maski; uint32_t masku;} u;
    u.masku = p_mask;
    gls_set_state_int(GL_STENCIL_WRITEMASK, 0, u.maski);
    gls_set_state_int(GL_STENCIL_BACK_WRITEMASK, 0, u.maski);
    real(p_mask);
...
glClearStencil: //GLint p_s
    //TODO: Mask p_s?
    gls_set_state_int(GL_STENCIL_CLEAR_VALUE, 0, p_s);
    real(p_s);

glClearColor: //GLfloat p_red, GLfloat p_green, GLfloat p_blue, GLfloat p_alpha
    gls_set_state_float(GL_COLOR_CLEAR_VALUE, 0, p_red);
    gls_set_state_float(GL_COLOR_CLEAR_VALUE, 1, p_green);
    gls_set_state_float(GL_COLOR_CLEAR_VALUE, 2, p_blue);
    gls_set_state_float(GL_COLOR_CLEAR_VALUE, 3, p_alpha);
    real(p_red, p_green, p_blue, p_alpha);

glClearDepth: //GLdouble p_depth
    gls_set_state_float(GL_DEPTH_CLEAR_VALUE, 0, fmin(fmax(p_depth, 0.0f), 1.0f));
    real(p_depth);

glClearDepthf: //GLfloat p_d
    gls_set_state_float(GL_DEPTH_CLEAR_VALUE, 0, fminf(fmaxf(p_d, 0.0f), 1.0f));
    real(p_d);

glDepthRange: //GLdouble p_near, GLdouble p_far
    for (int i = 0; i < gls_get_state_int(GL_MAX_VIEWPORTS, 0); i++) {
        gls_set_state_float(GL_DEPTH_RANGE, i*2+0, p_near);
        gls_set_state_float(GL_DEPTH_RANGE, i*2+1, p_far);
    }
    real(p_near, p_far);

glDepthRangef: //GLfloat p_n, GLfloat p_f
    for (int i = 0; i < gls_get_state_int(GL_MAX_VIEWPORTS, 0); i++) {
        gls_set_state_float(GL_DEPTH_RANGE, i*2+0, p_n);
        gls_set_state_float(GL_DEPTH_RANGE, i*2+1, p_f);
    }
    real(p_n, p_f);

glDepthRangeArrayv: //GLuint p_first, GLsizei p_count, const GLdouble* p_v
    int max_viewports = gls_get_state_int(GL_MAX_VIEWPORTS, 0);
    if (p_count<0 || p_first+p_count>max_viewports) ERROR("Invalid first and count");
    for (GLuint i = p_first*2; i < (p_first+p_count)*2; i++)
        gls_set_state_float(GL_DEPTH_RANGE, i, p_v[i]);
    real(p_first, p_count, p_v);

glPointSize: //GLfloat p_size
    if (p_size <= 0.0f) ERROR("size is not greater than zero");
    gls_set_state_float(GL_POINT_SIZE, 0, p_size);
    real(p_size);

glLineWidth: //GLfloat p_width
    if (p_width <= 0.0f) ERROR("width is not greater than zero");
    gls_set_state_float(GL_LINE_WIDTH, 0, p_width);
    real(p_width);

glPolygonOffset: //GLfloat p_factor, GLfloat p_units
    gls_set_state_float(GL_POLYGON_OFFSET_UNITS, 0, p_units);
    gls_set_state_float(GL_POLYGON_OFFSET_FACTOR, 0, p_factor);
    real(p_factor, p_units);

glPolygonOffsetClamp: //GLfloat p_factor, GLfloat p_units, GLfloat p_clamp
    gls_set_state_float(GL_POLYGON_OFFSET_UNITS, 0, p_units);
    gls_set_state_float(GL_POLYGON_OFFSET_FACTOR, 0, p_factor);
    gls_set_state_float(GL_POLYGON_OFFSET_CLAMP, 0, p_clamp);
    real(p_factor, p_units, p_clamp);

glSampleCoverage: //GLfloat p_value, GLboolean p_invert
    gls_set_state_float(GL_SAMPLE_COVERAGE_VALUE, 0, p_value);
    gls_set_state_bool(GL_SAMPLE_COVERAGE_INVERT, 0, p_invert);
    real(p_value, p_invert);

glStencilFunc: //GLenum p_func, GLint p_ref, GLuint p_mask
    union {int32_t maski; uint32_t masku;} u;
    u.masku = p_mask;
    gls_set_state_enum(GL_STENCIL_FUNC, 0, p_func);
    gls_set_state_int(GL_STENCIL_REF, 0, p_ref);
    gls_set_state_int(GL_STENCIL_VALUE_MASK, 0, u.maski);
    gls_set_state_enum(GL_STENCIL_BACK_FUNC, 0, p_func);
    gls_set_state_int(GL_STENCIL_BACK_REF, 0, p_ref);
    gls_set_state_int(GL_STENCIL_BACK_VALUE_MASK, 0, u.maski);
...
glStencilOp: //GLenum p_fail, GLenum p_zfail, GLenum p_zpass
    gls_set_state_enum(GL_STENCIL_FAIL, 0, p_fail);
    gls_set_state_enum(GL_STENCIL_PASS_DEPTH_FAIL, 0, p_zfail);
    gls_set_state_enum(GL_STENCIL_PASS_DEPTH_PASS, 0, p_zpass);
    gls_set_state_enum(GL_STENCIL_BACK_FAIL, 0, p_fail);
    gls_set_state_enum(GL_STENCIL_BACK_PASS_DEPTH_FAIL, 0, p_zfail);
    gls_set_state_enum(GL_STENCIL_BACK_PASS_DEPTH_PASS, 0, p_zpass);
    real(p_fail, p_zfail, p_zpass);

glStencilOpSeparate: //GLenum p_face, GLenum p_sfail, GLenum p_dpfail, GLenum p_dppass
    if (p_face==GL_FRONT || p_face==GL_FRONT_AND_BACK) {
        gls_set_state_enum(GL_STENCIL_FAIL, 0, p_sfail);
        gls_set_state_enum(GL_STENCIL_PASS_DEPTH_FAIL, 0, p_dpfail);
        gls_set_state_enum(GL_STENCIL_PASS_DEPTH_PASS, 0, p_dppass);
    }
    if (p_face==GL_BACK || p_face==GL_FRONT_AND_BACK) {
        gls_set_state_enum(GL_STENCIL_BACK_FAIL, 0, p_sfail);
        gls_set_state_enum(GL_STENCIL_BACK_PASS_DEPTH_FAIL, 0, p_dpfail);
        gls_set_state_enum(GL_STENCIL_BACK_PASS_DEPTH_PASS, 0, p_dppass);
    }
    real(p_face, p_sfail, p_dpfail, p_dppass);

glBlendFunc: //GLenum p_sfactor, GLenum p_dfactor
    for (size_t i = 0; i < gls_get_state_int(GL_MAX_DRAW_BUFFERS, 0); i++) {
        gls_set_state_enum(GL_BLEND_SRC_RGB, i, p_sfactor);
        gls_set_state_enum(GL_BLEND_DST_RGB, i, p_dfactor);
        gls_set_state_enum(GL_BLEND_SRC_ALPHA, i, p_sfactor);
        gls_set_state_enum(GL_BLEND_DST_ALPHA, i, p_dfactor);
    }
...
glBlendFuncSeparate: //GLenum p_sfactorRGB, GLenum p_dfactorRGB, GLenum p_sfactorAlpha, GLenum p_dfactorAlpha
    for (size_t i = 0; i < gls_get_state_int(GL_MAX_DRAW_BUFFERS, 0); i++) {
        gls_set_state_enum(GL_BLEND_SRC_RGB, i, p_sfactorRGB);
        gls_set_state_enum(GL_BLEND_DST_RGB, i, p_dfactorRGB);
        gls_set_state_enum(GL_BLEND_SRC_ALPHA, i, p_sfactorAlpha);
        gls_set_state_enum(GL_BLEND_DST_ALPHA, i, p_dfactorAlpha);
    }
    real(p_sfactorRGB, p_dfactorRGB, p_sfactorAlpha, p_dfactorAlpha);

glBlendFunci: //GLuint p_buf, GLenum p_src, GLenum p_dst
    if (p_buf < gls_get_state_int(GL_MAX_DRAW_BUFFERS, 0)) {
        gls_set_state_enum(GL_BLEND_SRC_RGB, p_buf, p_src);
        gls_set_state_enum(GL_BLEND_DST_RGB, p_buf, p_dst);
        gls_set_state_enum(GL_BLEND_SRC_ALPHA, p_buf, p_src);
        gls_set_state_enum(GL_BLEND_DST_ALPHA, p_buf, p_dst);
    }
...
glBlendFuncSeparatei: //GLuint p_buf, GLenum p_srcRGB, GLenum p_dstRGB, GLenum p_srcAlpha, GLenum p_dstAlpha
    if (p_buf < gls_get_state_int(GL_MAX_DRAW_BUFFERS, 0)) {
        gls_set_state_enum(GL_BLEND_SRC_RGB, p_buf, p_srcRGB);
        gls_set_state_enum(GL_BLEND_DST_RGB, p_buf, p_dstRGB);
        gls_set_state_enum(GL_BLEND_SRC_ALPHA, p_buf, p_srcAlpha);
        gls_set_state_enum(GL_BLEND_DST_ALPHA, p_buf, p_dstAlpha);
    }
...
glBlendColor: //GLfloat p_red, GLfloat p_green, GLfloat p_blue, GLfloat p_alpha
    gls_set_state_float(GL_BLEND_COLOR, 0, p_red);
    gls_set_state_float(GL_BLEND_COLOR, 1, p_green);
    gls_set_state_float(GL_BLEND_COLOR, 2, p_blue);
    gls_set_state_float(GL_BLEND_COLOR, 3, p_alpha);
    real(p_red, p_green, p_blue, p_alpha);

glBlendEquation: //GLenum p_mode
    for (size_t i = 0; i < gls_get_state_int(GL_MAX_DRAW_BUFFERS, 0); i++) {
        gls_set_state_enum(GL_BLEND_EQUATION_RGB, i, p_mode);
        gls_set_state_enum(GL_BLEND_EQUATION_ALPHA, i, p_mode);
    }
    real(p_mode);

glBlendEquationSeparate: //GLenum p_modeRGB, GLenum p_modeAlpha
    for (size_t i = 0; i < gls_get_state_int(GL_MAX_DRAW_BUFFERS, 0); i++) {
        gls_set_state_enum(GL_BLEND_EQUATION_RGB, i, p_modeRGB);
        gls_set_state_enum(GL_BLEND_EQUATION_ALPHA, i, p_modeAlpha);
    }
    real(p_modeRGB, p_modeAlpha);

glBlendEquationi: //GLuint p_buf, GLenum p_mode
    if (p_buf < gls_get_state_int(GL_MAX_DRAW_BUFFERS, 0)) {
        gls_set_state_enum(GL_BLEND_EQUATION_RGB, p_buf, p_mode);
        gls_set_state_enum(GL_BLEND_EQUATION_ALPHA, p_buf, p_mode);
    }
    real(p_buf, p_mode);

glBlendEquationSeparatei: //GLuint p_buf, GLenum p_modeRGB, GLenum p_modeAlpha
    if (p_buf < gls_get_state_int(GL_MAX_DRAW_BUFFERS, 0)) {
        gls_set_state_enum(GL_BLEND_EQUATION_RGB, p_buf, p_modeRGB);
        gls_set_state_enum(GL_BLEND_EQUATION_ALPHA, p_buf, p_modeAlpha);
    }
    real(p_buf, p_modeRGB, p_modeAlpha);

glViewport: //GLint p_x, GLint p_y, GLsizei p_width, GLsizei p_height
    gls_set_state_float(GL_VIEWPORT, 0, p_x);
    gls_set_state_float(GL_VIEWPORT, 1, p_y);
    gls_set_state_float(GL_VIEWPORT, 2, p_width);
    gls_set_state_float(GL_VIEWPORT, 3, p_height);
    real(p_x, p_y, p_width, p_height);

glViewportIndexedf: //GLuint p_index, GLfloat p_x, GLfloat p_y, GLfloat p_w, GLfloat p_h
    gls_set_state_float(GL_VIEWPORT, p_index*4+0, p_x);
    gls_set_state_float(GL_VIEWPORT, p_index*4+1, p_y);
    gls_set_state_float(GL_VIEWPORT, p_index*4+2, p_w);
    gls_set_state_float(GL_VIEWPORT, p_index*4+3, p_h);
...
glViewportArrayv: //GLuint p_first, GLsizei p_count, const GLfloat* p_v
    for (size_t i = p_first*4; i < (p_first+p_count)*4; i++)
        gls_set_state_float(GL_VIEWPORT, i, p_v[i]);
    real(p_first, p_count, p_v);

glViewportIndexedfv: //GLuint p_index, const GLfloat* p_v
    for (size_t i = 0; i < 4; i++)
        gls_set_state_float(GL_VIEWPORT, p_index*4+i, p_v[i]);
    real(p_index, p_v);

glScissor: //GLint p_x, GLint p_y, GLsizei p_width, GLsizei p_height
    gls_set_state_int(GL_SCISSOR_BOX, 0, p_x);
    gls_set_state_int(GL_SCISSOR_BOX, 1, p_y);
    gls_set_state_int(GL_SCISSOR_BOX, 2, p_width);
    gls_set_state_int(GL_SCISSOR_BOX, 3, p_height);
    real(p_x, p_y, p_width, p_height);

glScissorIndexed: //GLuint p_index, GLint p_left, GLint p_bottom, GLsizei p_width, GLsizei p_height
    gls_set_state_int(GL_SCISSOR_BOX, p_index*4+0, p_left);
    gls_set_state_int(GL_SCISSOR_BOX, p_index*4+1, p_bottom);
    gls_set_state_int(GL_SCISSOR_BOX, p_index*4+2, p_width);
    gls_set_state_int(GL_SCISSOR_BOX, p_index*4+3, p_height);
    real(p_index, p_left, p_bottom, p_width, p_height);

glScissorArrayv: //GLuint p_first, GLsizei p_count, const GLint* p_v
    for (size_t i = p_first*4; i < (p_first+p_count)*4; i++)
        gls_set_state_float(GL_SCISSOR_BOX, i, p_v[i]);
    real(p_first, p_count, p_v);

glScissorIndexedv: //GLuint p_index, const GLint* p_v
    for (size_t i = 0; i < 4; i++)
        gls_set_state_int(GL_SCISSOR_BOX, p_index*4+i, p_v[i]);
    real(p_index, p_v);

glHint: //GLenum p_target, GLenum p_mode
    gls_set_hints(p_target, p_mode);
    real(p_target, p_mode);

glProvokingVertex: //GLenum p_mode
    gls_set_state_enum(GL_PROVOKING_VERTEX, 0, p_mode);
    real(p_mode);

glLogicOp: //GLenum p_opcode
    gls_set_state_enum(GL_LOGIC_OP_MODE, 0, p_opcode);
    real(p_opcode);

glPrimitiveRestartIndex: //GLuint p_index
    gls_set_state_int(GL_PRIMITIVE_RESTART_INDEX, 0, p_index);
    real(p_index);

glPolygonMode: //GLenum p_face, GLenum p_mode
    gls_set_state_enum(GL_POLYGON_MODE, 0, p_mode);
    real(p_face, p_mode);

glCullFace: //GLenum p_mode
    gls_set_state_enum(GL_CULL_FACE_MODE, 0, p_mode);
    real(p_mode);

glFrontFace: //GLenum p_mode
    gls_set_state_enum(GL_FRONT_FACE, 0, p_mode);
    real(p_mode);

glDepthFunc: //GLenum p_func
    gls_set_state_enum(GL_DEPTH_FUNC, 0, p_func);
    real(p_func);
...
glPointParameterf: //GLenum p_pname, GLfloat p_param
    switch (p_pname) {
    case GL_POINT_FADE_THRESHOLD_SIZE: gls_set_state_float(p_pname, 0, p_param);
    case GL_POINT_SPRITE_COORD_ORIGIN: gls_set_state_enum(p_pname, 0, p_param);
    }
...
glPointParameterfv: //GLenum p_pname, const GLfloat* p_params
    switch (p_pname) {
    case GL_POINT_FADE_THRESHOLD_SIZE: gls_set_state_float(p_pname, 0, p_params[0]);
    case GL_POINT_SPRITE_COORD_ORIGIN: gls_set_state_enum(p_pname, 0, p_params[0]);
    }
...
glClipControl: //GLenum p_origin, GLenum p_depth
    gls_set_state_enum(GL_CLIP_ORIGIN, 0, p_origin);
    gls_set_state_enum(GL_CLIP_DEPTH_MODE, 0, p_depth);

glMinSampleShading: //GLfloat p_value
    gls_set_state_float(GL_MIN_SAMPLE_SHADING_VALUE, 0, p_value);
    real(p_value);
...
glPatchParameteri: //GLenum p_pname, GLint p_value
    if (p_value<=0 || p_value>gls_get_state_int(GL_MAX_PATCH_VERTICES, 0))
        ERROR("Invalid value");
